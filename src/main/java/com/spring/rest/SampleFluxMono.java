...
public class SampleFluxMono {
  ...
  public Flux<String> fluxString() {
    return Flux.fromIterable(List.of("Name 1", "Name 2", "Name 3")).map(String::toUpperCase).log();
  }
  ...
  public Flux<String> fluxString_transform(int strLength) {
    Function<Flux<String>, Flux<String>> fluxFunction =
        input -> input.map(String::toUpperCase).filter(x -> x.length() > strLength);
  ...
  public Flux<String> fluxString_transform_switchIfEmpty(int strLength) {
    ...
    var defaultFlux = Flux.just("default").transform(fluxFunction);
  ...
  public Mono<String> monoString() {
    return Mono.just("Name N").map(String::toUpperCase).log();
  }

  public Mono<List<String>> monoString_flatMap() {
    return Mono.just("Name N").map(String::toUpperCase).flatMap(this::splitStringMono).log();
  }
  ...
  public Flux<String> monoString_flatMapMany() {
    return Mono.just("Name N").map(String::toUpperCase).flatMapMany(this::splitString).log();
  }

  /** concat is a static function in Flux concatWith is an instance method in Flux and Mono */
  ...
  /** merge is a static function in Flux mergeWith is an instance method in Flux and Mono */
  public Flux<String> fluxString_merge() {
    var firstFlux = Flux.just("Name 1", "Name 2").delayElements(Duration.ofMillis(100)).log();
    var secondFlux = Flux.just("Name 3", "Name 4").delayElements(Duration.ofMillis(110)).log();
  ...
  /** zip is a static function in Flux zipWith is an instance method in Flux and Mono */
  public Flux<String> fluxString_zip() {
    var firstFlux = Flux.just("Name 1", "Name 2");
    var secondFlux = Flux.just("Name 3", "Name 4");

    return Flux.zip(firstFlux, secondFlux, (firstELe, secondEle) -> firstELe + secondEle).log();
  }
  ...
  public Flux<String> fluxString_zipWith() {
    ...
    return firstFlux.zipWith(secondFlux, (firstELe, secondEle) -> firstELe + secondEle).log();
  }
  ...
  public Mono<String> monoString_zipWith() {
    ...
    return firstFlux.zipWith(secondFlux).map(t2 -> t2.getT1() + t2.getT2()).log();
  }
}
